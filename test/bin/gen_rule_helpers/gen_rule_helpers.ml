...
let pp_enabled_if fmt = function
  | Some exp -> Fmt.pf fmt " (enabled_if %s)" exp
  | None -> ()

let pr_runtest_alias ?enabled_if dir =
  Fmt.pr {|
(rule
 (alias runtest)%a
 (action (diff %s/%s %s)))
|} pp_enabled_if enabled_if dir.dir_name
    dir.expected_file dir.target_file

let pr_rule ?enabled_if ~pp_action dir =
  Fmt.pr
    {|
(rule
 (target %s)%a
 ...
 (action%a))
|}
    dir.target_file pp_enabled_if enabled_if dir.dir_name pp_action dir
...
let pr_rule ?enabled_if ~pp_action dir_name =
  let dir = dir dir_name in
  pr_rule ?enabled_if ~pp_action dir;
  pr_runtest_alias ?enabled_if dir

let rule_gen ?enabled_if generator rule_type () =
  let pp_action =
  ...
  let dirs =
    List.filter
      ...
      (get_dirs ".")
  in
  List.iter (pr_rule ?enabled_if ~pp_action) dirs

let run ?enabled_if generator =
  let open Cmdliner in
  let cmds =
    Term.
      [
        ( const (rule_gen ?enabled_if generator `Test_expect) $ const (),
          info "test_expect" );
        ( const (rule_gen ?enabled_if generator `Test_failure) $ const (),
          info "test_failure" );
      ]
  in
  let default =
